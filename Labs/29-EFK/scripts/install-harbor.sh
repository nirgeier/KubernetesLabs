...
# Step 2: Load infrastructure and Harbor images if available (offline)
print_step "Step 2: Loading images from local artifacts..."

# Load the local-path-provisioner helper pod image first; without it
# the provisioner cannot create PVCs and all StatefulSets stay Pending.
...
if [[ -n "$NODE_IP" ]]; then
  # Add entries directly to /etc/hosts on the host (no internet image needed).
  # Falls back to a manual instruction if /etc/hosts is not writable.
  INGRESS_HOSTS=("${HARBOR_DOMAIN}" "${ELASTICSEARCH_INGRESS_HOST:-elasticsearch.local}" "${KIBANA_INGRESS_HOST:-kibana.local}")
  for HOST in "${INGRESS_HOSTS[@]}"; do
    if grep -q "${HOST}" /etc/hosts 2>/dev/null; then
      print_info "/etc/hosts already has entry for ${HOST}"
