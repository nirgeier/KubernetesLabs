...
# Infrastructure image used by local-path-provisioner helper pods to provision
# PVCs (must be available offline or Harbor StatefulSets will stay Pending)
INFRA_IMAGES=(
  "rancher/mirrored-library-busybox:1.36.1"
)

HARBOR_TAR="${HARBOR_CHARTS_DIR}/harbor-images.tar"
INFRA_TAR="${HARBOR_CHARTS_DIR}/infra-images.tar"
...
print_info "Infrastructure images (local-path-provisioner helper pod):"
...
ALL_IMAGES=("${HARBOR_IMAGES[@]}" "${INFRA_IMAGES[@]}")
...
echo ""
print_step "Saving Harbor images to tar..."
...
print_step "Saving infrastructure images to tar..."
docker save "${INFRA_IMAGES[@]}" -o "$INFRA_TAR"
SIZE=$(du -h "$INFRA_TAR" | cut -f1)
print_success "Infrastructure images saved: $(basename "$INFRA_TAR") ($SIZE)"
